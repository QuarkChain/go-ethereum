...
func ApplyTransactionForTest(msg types.Message, config *params.ChainConfig, bc ChainContext, author *common.Address, gp *GasPool, statedb *state.StateDB, blockNumber *big.Int, blockHash common.Hash, tx *types.Transaction, usedGas *uint64, evm *vm.EVM, expectExternalCallResult []byte) (*types.Receipt, []byte, error) {
	return applyTransaction(msg, config, bc, author, gp, statedb, blockNumber, blockHash, tx, usedGas, evm, expectExternalCallResult)
}
