...
func BenchmarkApplyTransactionWithCallResult(b *testing.B) {
	...
	for i := 0; i < b.N; i++ {
		...
		if !bytes.Equal(cs, wrapTx.ExpectCCRBytes) {
			b.Fatalf("cross_chain_result err, ")
		}
...
func BenchmarkApplyTransactionWithCallResultThroughIndependentClient(b *testing.B) {
	var (
		config = &params.ChainConfig{
			ChainID:             big.NewInt(3334),
			HomesteadBlock:      big.NewInt(0),
	...
	externalClient, err := ethclient.Dial("http://127.0.0.1:8545")
	if err != nil {
		b.Error(err)
	}

	const localTxHash = "0xf2ca5b8acc369bb7a6b8592447b09b5cc83ffbec7ab6fe5d1299c7811e227dcd"
	Args_CallOnce := NewCrossChainCallArgment(config, externalClient, 4, common.HexToHash(localTxHash), 0, 300, 10)
		...
		wrapTx :=
			&WrapTx{
				Tx: types.NewTx(&types.DynamicFeeTx{
					...
					GasTipCap: big.NewInt(1000000000),
					GasFeeCap: big.NewInt(6000000000),
					Data:      common.FromHex(""),
				}),
				Args:           Args_CallOnce,
				ExpectTxData:   Args_CallOnce.CallOncePack,
				ExpectCCRBytes: common.FromHex(""),
			}
